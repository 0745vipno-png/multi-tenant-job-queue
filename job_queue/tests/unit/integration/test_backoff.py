...
#關於這段代碼額外問答補充 
#問:「這段對 Full Jitter 寫測試，而且還用了 set(samples) > 90 來測隨機性。這想法很有趣，當時是怎麼想到要這樣測一個非確定性的演算法的？」
#答:「因為在分散式環境下，Full Jitter 是我們用來對抗『驚群效應』的最後防線。如果隨機函數因為代碼重構或 Bug 喪失了隨機性（例如退化成固定值），那重試流量就會再度同步，引發系統雪崩。
#我認為『沒測試過的演算法，就是壞掉的演算法』。所以我採用 100 次多點採樣，一方面校驗其邊界嚴格落在 [0, delay] 區間，另一方面利用 Python 的 set 去重統計。如果去重後的元素數量小於 90，代表隨機度（Entropy）不夠分散，系統會自動在 CI/CD 階段噴報錯攔截。這樣我才能 100% 保證這條防線在生產環境是絕對安全的。」
